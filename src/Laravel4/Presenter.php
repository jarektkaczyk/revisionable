...
use Sofa\Revisionable\Presenter as Abstract;

class Presenter extends Abstract {
	...
	{
		if ($revision instanceof Revision) $revision = $revision->getAttributes();
	...
	public function renderDiff()
	{
		...
		foreach ($this->getDiff() as $key => $values)
		{
			...
			$html .= str_replace([':key', ':old', ':new'], [$key, $this->old($key), $this->new($key)], Config::get('revisionable::templates.diff.body'));
